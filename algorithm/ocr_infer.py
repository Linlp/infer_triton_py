...
class ClientInfer(object):
    def __init__(self):
        self.url = ""
        self.client = None

        self.infer_model_name = "yolov5"
        self.infer_input_names = "image"
        self.infer_output_names = "output"

    def __get_os(self):
        os_name = platform.system()
        if os_name == "Windows":
            self.url = "192.168.0.33:8000"
        elif os_name == "Linux":
            self.url = "0.0.0.0:8000"
        else:
            self.url = "0.0.0.0:8000"

    def set_url(self, ip:str, port:int):
        self.url = f"{ip}:{port}"

    def infer_client(self):
        self.__get_os()
        self.client = httpclient.InferenceServerClient(url=self.url)

    def set_infer_name(self, model_name, input_names, output_names):
        self.infer_input_names = input_names
        self.infer_output_names = output_names
        self.infer_model_name = model_name

    # def triton_infer(self, image_preprocessed, model_name="yolov5", input_name='images', output_name='output0'):
    def __triton_infer(self, image_preprocessed, model_name="yolov5", input_name='images', output_name='output0'):
        self.infer_client()
        triton_input = httpclient.InferInput(input_name, image_preprocessed.shape, "FP32")
        triton_input.set_data_from_numpy(image_preprocessed)
        response = self.client.infer(model_name=model_name, inputs=[triton_input])
        pred_results = response.as_numpy(output_name)
        return pred_results

    def infer(self, image_preprocessed):
        return self.__triton_infer(image_preprocessed, self.infer_model_name, self.infer_input_names, self.infer_output_names)

# client = ClientInfer().infer_client()
...
class TextDetector(object):
    def __init__(self, model_path, mask_thresh=0.3, box_thresh=0.6,
        ...
        self.predictor = None
        self.triton_client = ClientInfer()
    ...
    def forward(self, input):
        ...
        # output = self.predictor.run(None, {"x": input})[0]
        self.triton_client.set_infer_name("pp_v4_det", "x","sigmoid_11.tmp_0")
        output = self.triton_client.infer(input)
        return output
...
class TextClassifier(object):
    ...
    def forward(self, inputs):
        ...
        # output = self.predictor.run(None, {"x": inputs})[0]
        # return output
         pass
...
class TextRecognizer(object):
    ...
    def forward(self, inputs):
        ...
        self.triton_infer.set_infer_name("pp_v4_rec", "x", "softmax_2.tmp_0")
        output = self.triton_infer.infer(inputs)
        return output
...
class TritonOCRInfer(object):
    def __init__(self):
       pass

    def infer(self, image, det_flag = True):
        ...
        # 3 text recognition
        character_dict_path = "../ocr/ppocr_keys_v1.txt"
        text_recognizer = TextRecognizer(None, character_dict_path)
        imgs_pre_batch, indices = text_recognizer.preprocess(img_list)
        rec_txts = text_recognizer.postprocess(imgs_pre_batch, indices)

        results = []
        flag = det_flag
        for det, rec in zip(det_boxes, rec_txts):
            if flag:
                result = [det, rec]
...
def triton_test_infer():
    image = cv2.imread("../datas/ocr_test.jpg")
    infer = TritonOCRInfer()
    results = infer.infer(image)
    print(results)


def ocr_triton_infer():
    from loguru import logger

    image = cv2.imread("../datas/ocr_test.jpg")
    src_h, src_w, _ = image.shape

    det_model_file_path = "../ocr/onnx/ppv4_det.onnx"
    cls_model_file_path = "../ocr/onnx/ppv4_cls.onnx"
    rec_model_file_path = "../ocr/onnx/ppv4_rec.onnx"
    character_dict_path = "../ocr/ppocr_keys_v1.txt"
    font_path = "../ocr/simfang.ttf"

    # 1. text detection
    logger.info("text detection")
    text_detector = TextDetector(det_model_file_path)
    img_pre = text_detector.preprocess(image)
    from algorithm.triton_infer import ocr_det_infer
    det_pred = ocr_det_infer(img_pre)
    # det_pred = text_detector.forward(img_pre)
    det_boxes = text_detector.postprocess(det_pred, src_h, src_w)
    logger.info("text postprocess")
    ...
    imgs_pre_batch, indices = text_recognizer.preprocess(img_list)
    logger.info("text rec postprocess start!")
    rec_txts = text_recognizer.postprocess(imgs_pre_batch, indices, False)
    logger.info("text rec postprocess end!")
    # 4. visualization
    ...
    # cv2.imwrite("result_test.jpg", draw_img[:, :, ::-1])
